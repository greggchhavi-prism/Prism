exports.handler = async function(event) {
  ...
  const TABLE_ID = 'tblBl2L3ROrf1bL0I';
  ...
  try {
    ...
    console.log('Airtable data:', JSON.stringify(data));
    if (!data.records || data.records.length === 0) {
      return { statusCode: 404, body: JSON.stringify({ error: 'Profile not found', debug: { type, url: url.replace(AIRTABLE_TOKEN, 'REDACTED'), data } }) };
    }
    return {
      statusCode: 200,
